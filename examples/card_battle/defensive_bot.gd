## Example bot: blocks first to absorb incoming damage, then attacks with remaining focus.
...
func choose_action(state: Variant, available_actions: Array) -> Variant:
	...
	for action in available_actions:
		...
		if act["action"] == "play_card":
			...
			if card["type"] == "block" and card["value"] > best_block_value:
				best_block = action
				best_block_value = card["value"]
			elif card["type"] == "attack" and card["value"] > best_attack_value:
				best_attack = action
